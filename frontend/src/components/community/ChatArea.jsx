...
const ChatArea = () => {
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-baseline gap-3 mb-1">
                            <span className={`font-bold text-[15px] ${isOwn ? 'text-primary' : 'text-slate-700'} hover:underline cursor-pointer tracking-tight`}>
                                {msg.user_email?.split('@')[0] || 'User'}
                                {isOwn && <span className="text-[11px] font-medium text-indigo-500/70 ml-2 uppercase tracking-wider bg-primary/10 px-1.5 py-0.5 rounded">You</span>}
                            </span>
                            <span className="text-xs text-slate-400 font-medium">{formatDate(msg.timestamp)}</span>
                        </div>
                        ...
                        {isEditing ? (
                            ...
                            <div className="text-slate-600 text-[15px] leading-relaxed whitespace-pre-wrap break-words">
                                {msg.content}
                            </div>
                        )}
                        ...
                        {!isEditing && !isDeleting && (
                            <div className={`flex items-center gap-3 mt-2 transition-all duration-200 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'}`}>
                                ...
                                <div className="flex items-center bg-slate-50 rounded-md px-2 py-1 border border-slate-200 gap-1.5 shadow-sm">
                                    ...
                                    <button
                                        className={`p-1 rounded-md transition-all ${userVote === -1 ? 'text-rose-400 bg-rose-400/10' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}
                                        onClick={() => handleVote(msg.id, userVote, -1)}
                                    >
        ...
        return (
            <div className="flex-1 flex flex-col h-full relative min-w-0 bg-slate-50">
                ...
                <div className="h-16 flex items-center px-6 border-b border-slate-200 shrink-0 bg-slate-50/80 backdrop-blur-xl shadow-sm z-10">
                    ...
                    <span className="text-slate-800 font-extrabold text-lg tracking-tight truncate flex-1">
                        @{currentDM.other_user_email?.split('@')[0] || 'User'}
                    </span>
                </div>

                {/* Messages */}
                <div className="flex-1 overflow-y-auto px-6 py-8 space-y-6 custom-scrollbar bg-gradient-to-b from-slate-50 to-slate-100/50">
                        ...
                        return (
                            <div key={msg.id || index} className={`flex gap-3 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
                                ...
                                <div className={`flex flex-col max-w-[70%] ${isOwn ? 'items-end' : 'items-start'}`}>
                                    ...
                                    <span className="text-[11px] font-medium text-slate-400 mt-1.5 px-1">{formatTime(msg.timestamp)}</span>
                                </div>
                            </div>
                        );
                    })}
                ...
                <div className="px-6 pb-6 pt-4 shrink-0 bg-slate-50">
                    ...
                    >
                        <input
                            type="text"
                            className="flex-1 bg-transparent border-none focus:outline-none text-slate-700 placeholder-slate-500 py-2 text-[15px]"
    ...
    return (
        <div className="flex-1 flex h-full relative overflow-hidden min-w-0 bg-slate-50">
            <div className="flex-1 flex flex-col h-full relative min-w-0">
                {/* Channel Header */}
                <div className="h-16 flex items-center justify-between px-6 border-b border-slate-200 shrink-0 bg-slate-50/80 backdrop-blur-xl shadow-sm z-10">
                    <div className="flex items-center gap-3 overflow-hidden">
                        <span className="text-slate-400 font-light text-2xl">#</span>
                        <div className="flex flex-col min-w-0">
                            <h2 className="font-extrabold text-slate-800 text-lg tracking-tight truncate">{currentChannel.name}</h2>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                        ...
                        >
                            <ClipboardList size={18} className="mr-2" /><span className="hidden sm:inline">Reviews</span>
                        </button>
                        <button
                            className="flex items-center gap-2 px-4 py-2 bg-white text-slate-600 hover:text-slate-800 rounded-md border border-slate-200 hover:border-slate-600 transition-all text-sm font-bold shadow-sm"
                            onClick={() => setShowNotes(true)}
                        >
                            <FileText size={18} className="mr-2" /><span className="hidden sm:inline">Notes</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="px-6 pb-6 pt-2 shrink-0 bg-slate-50">
                    <div className="bg-white rounded-md border border-slate-200 focus-within:border-primary/30 focus-within:ring-1 focus-within:ring-primary/30 transition-all shadow-inner overflow-hidden">
                        ...
                        <form className="flex items-end gap-3 px-5 py-3" onSubmit={handleSend}>
                            ...
                            <textarea
                                ref={inputRef}
                                className="flex-1 bg-transparent border-none focus:outline-none text-slate-700 placeholder-slate-500 py-2.5 text-[15px] resize-none max-h-40"
            ...
            {/* Thread Panel */}
            {activeThread && (
                <div className="w-96 border-l border-slate-200 bg-slate-50 flex flex-col h-full shadow-[-20px_0_40px_rgba(0,0,0,0.3)] absolute right-0 top-0 z-30 sm:relative sm:shadow-none">
                    <div className="h-16 flex items-center justify-between px-6 border-b border-slate-200 bg-slate-50/80 backdrop-blur-xl shrink-0">
                        <div>
                            <h3 className="font-extrabold text-slate-800 text-[15px] tracking-tight">Thread</h3>
                        ...
                        {loadingThread ? (
                            ...
                            <>
                                ...
                                {threadData.replies.map((reply, index) => (
                                    <div key={reply.id} className="pl-4 border-l-2 border-primary/20 ml-6 mr-2 py-1 mb-2 hover:border-primary/30 transition-colors">
